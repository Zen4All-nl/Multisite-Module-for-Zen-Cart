...
<div style="text-align:center;vertical-align:middle;margin-bottom:20px;">
	<a href="?action=display_config">Display Configuration for a new site</a> -
	<a href="?action=categories_sites">Display the relations Categories/Sites</a>
</div>
<div style="text-align:center;vertical-align:middle;margin-bottom:20px;">
<?php
if(isset($_GET['action'])) {
	switch($_GET['action']) {
		case 'display_config':
		?>
			List of the config that you can override from a config file from includes/config_sites/*_config.php<br />
			You don't need to copy all of them, you can only copy the one you wish to modify. <br /><br />
		...
		case 'categories_sites':
			...
			<h1>Categories/Sites</h1>
			
			You can use this tool in order to allocate a category to a site.<br/>
			All you have to do is to write the SITE_NAME (defined in the folder config_sites) in the collomn Sites.<br/>
			If a categories has to be seen by more than one site, then you have to write the sites separated by a dash.<br/>
			Example: site1-site2-site3<br/>
			<br/>
			If you want a sub-category  to be seen on a site, you should also add its parent category.<br/><br/>
			Add This site to all Categories <input id="txt_site" type="text" size="32" />
			<input type="button" onclick="add_to_all_cats()" value="Add to all" />
			<input type="button" onclick="remove_from_all_cats()" value="Remove from all" />
			...
			$display_sites = array();
			$display_sites[] = '<a href="'.zen_href_link('multisite','action='.$_GET['action']).'">All</a> ('.$total_cats.')';
			...
			<tr style="text-align:center;font-weight:bold;"><td>Categories</td><td>sites</td></tr>
			<?php
